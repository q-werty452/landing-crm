...
(function () {
  ...
  // Конфигурация кубов: большие (фон), средние, маленькие (передний план)
  const CONFIGS = [
  ...
  const cubes = CONFIGS.map(cfg => {
    ...
    // Прозрачность зависит от глубины: ближние кубы ярче
    const opacity = 0.28 + cfg.depth * 0.52;
    el.style.cssText = `width:${cfg.size}px;height:${cfg.size}px;left:${cfg.x}%;top:${cfg.y}%;opacity:${opacity}`;
  ...
  let scrollY = 0;
  let mouseX  = 0, mouseY  = 0; // текущие (сглаженные)
  let tMouseX = 0, tMouseY = 0; // целевые (от мыши)
  let t0 = null;
  ...
  function tick(ts) {
    ...
    // Плавная интерполяция позиции мыши (lerp)
    mouseX += (tMouseX - mouseX) * 0.06;
    mouseY += (tMouseY - mouseY) * 0.06;

    cubes.forEach(c => {
      // Плавающее смещение по синусоиде
      const floatY = Math.sin(elapsed * c.floatSpeed + c.floatOff) * 18;

      // Параллакс от скролла — дальние двигаются меньше
      const parScrollY = scrollY * 0.05 * (1 - c.depth * 0.8);

      // Реакция на курсор — ближние кубы двигаются сильнее
      const cursorStr = 0.5 + c.depth * 1.5; // диапазон: 0.57 — 1.55
      const parCurX   = mouseX * 40 * cursorStr;
      const parCurY   = mouseY * 30 * cursorStr;

      // Скорость вращения — ближние крутятся чуть быстрее
